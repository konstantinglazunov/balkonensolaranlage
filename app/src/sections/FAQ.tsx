...
const FAQ = () => {
  ...
  const faqConfig = getFaqConfig(t);
  ...
  const renderAnswerWithMastrLink = (answer: string) => {
    const tokens = ['MaStR', 'Marktstammdatenregister'];
    const hasToken = tokens.some((token) => answer.includes(token));

    if (!hasToken) {
      return answer;
    }

    const parts = answer.split(/(MaStR|Marktstammdatenregister)/g);

    return (
      <>
        {parts.map((part, index) => (
          <Fragment key={`mastr-part-${index}`}>
              ...
              >
                <div className="px-6 lg:px-8 py-6 bg-white border border-t-0 border-[#e9e9e9]">
                  <p className="text-[#696969] text-base font-light leading-relaxed">
                    {renderAnswerWithMastrLink(faq.answer)}
                  </p>
                </div>
              </div>
            </div>
