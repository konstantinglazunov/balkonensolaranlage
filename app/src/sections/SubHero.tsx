...
import { getSubHeroConfig } from '../config';
...
// Stat counter component to handle individual hook calls
const StatCounter = ({ 
  value, 
  suffix, 
  label, 
  ...
  return (
    <div className={`p-6 ${hasBorder ? 'border-r border-gray-200' : ''}`}>
...
const SubHero = () => {
  ...
  const subHeroConfig = getSubHeroConfig(t);
          ...
          >
            {subHeroConfig.stats.map((stat, index) => (
              <StatCounter
                ...
                hasBorder={index < subHeroConfig.stats.length - 1}
              />
            ))}
          </div>
        </div>
